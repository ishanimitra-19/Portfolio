...
# -*- coding: utf-8 -*-
"""
Long call option simulator under the Black–Scholes framework.
"""
...
def black_scholes_call(S: float, K: float, sigma: float, r: float, t: float) -> float:
    """Return the theoretical Black–Scholes price of a European call option."""
...
def gbm_terminal_samples(S0: float, mu: float, sigma: float, T: float, n: int, seed: int = None) -> np.ndarray:
    """Generate terminal asset prices under geometric Brownian motion."""
    rng = np.random.default_rng(seed) if seed is not None else np.random.default_rng()
...
# ----------------------------
# Core computations (LONG call)
# ----------------------------

def long_call_pl_per_share(premium_paid: float, K: float, ST: float) -> float:
    """Return the profit or loss per share for a long call position."""
    payoff = max(ST - K, 0.0)
    return payoff - premium_paid

def run_once(S, K, sigma, r, t, steps=252, mu_for_path=None, seed=None):
    """Simulate a single call trade; return theoretical price, path, terminal price, and P/L."""
    theo = black_scholes_call(S, K, sigma, r, t)  # theoretical price (cost)
    mu = r if mu_for_path is None else mu_for_path
    path = gbm_path(S, mu, sigma, steps, t, seed=seed)
    ST = float(path[-1])
    pl_per_share = long_call_pl_per_share(theo, K, ST)  # using theoretical cost
    return theo, path, ST, pl_per_share

def mc_pl_per_contract_vectorized(S, K, sigma, r, t, market_ask, n_paths=1_000_000, mu_for_mc=None, seed=None):
    """Vectorized Monte Carlo estimate of call P/L per contract."""
    mu = r if mu_for_mc is None else mu_for_mc
    ST = gbm_terminal_samples(S, mu, sigma, t, n_paths, seed=seed)
    payoff = np.maximum(ST - K, 0.0)
    pl_per_share = payoff - market_ask
    return (pl_per_share * 100.0)  # returns array length n_paths

def mc_mean_pl_streaming(S, K, sigma, r, t, market_ask, n_paths=1_000_000, batch_size=200_000, mu_for_mc=None, seed=None):
    """Compute the expected P/L per contract for a long call using streaming Monte Carlo."""
    mu = r if mu_for_mc is None else mu_for_mc
    rng = np.random.default_rng(seed) if seed is not None else np.random.default_rng()
    remaining = n_paths
    sum_pl = 0.0
...
def plot_single_path(path: np.ndarray, K: float, pl_per_share: float):
    """Plot a single simulated price path and highlight the terminal payoff region."""
    steps = len(path) - 1
    ST = path[-1]
    plt.figure()
    plt.title("Terminal Value of a Long Call Option")
...
def plot_cumulative_equity(sampled_contract_pls: np.ndarray, max_points: int = 50_000):
    """Plot the cumulative equity curve across sequential call trades."""
    ...
    plt.figure()
    plt.title("Trading the Long Call Over Time")
...
class App(tk.Tk):
    """Graphical user interface for simulating long call option trades."""

    def __init__(self):
        super().__init__()
        self.title("Long Call Simulator")
        ...
        # Form layout for parameter entry
        ...
        rows = [
            ...
            ("Market bid (for reference)", "market_bid"),
            ("Market ask (cost to buy)", "market_ask"),
            ("Path steps", "steps"),
            ("MC paths", "mc_paths"),
        ...
        # Button bar for running simulations
    ...
    def read_floats(self):
        """Parse input fields into numeric values with optional overrides for drift and seed."""
        f = {}
    ...
    def write(self, s: str):
        """Append a line of text to the output widget and scroll to the end."""
        self.out.insert("end", s + "\n")
        self.out.see("end")

    def on_run_once(self):
        """Handle the 'Run single path + price' button click."""
        try:
            p = self.read_floats()
            ...
            bid = p["market_bid"]
            ask = p["market_ask"]
            self.write(f"Theoretical call price (cost): {theo:.6f}")
            self.write(f"Market maker quote: {bid:.2f} @ {ask:.2f}")
            self.write(f"Terminal price S_T: {ST:.6f}")
            self.write(f"Long call P/L per share at S_T (using theoretical cost): {pl_share:.6f}")
            plot_single_path(path, p["K"], pl_share)
        except Exception as e:
            self.write(f"Error: {e}")

    def on_run_mc_stream(self):
        """Handle the 'Run Monte Carlo (mean only, streaming)' button click."""
        try:
            p = self.read_floats()
            exp_pl_contract = mc_mean_pl_streaming(
                p["S"], p["K"], p["sigma"], p["r"], p["t"], p["market_ask"],
                n_paths=p["mc_paths"], batch_size=p["mc_batch"], mu_for_mc=p["mu"], seed=p["seed"]
            )
            n_paths_val = p["mc_paths"]
            self.write(f"Expected P/L per contract at ask (streaming mean, n={n_paths_val:,}): {exp_pl_contract:.6f}")
        except Exception as e:
            self.write(f"Error: {e}")

    def on_run_mc_full(self):
        """Handle the 'Run Monte Carlo (full, plots)' button click."""
        try:
            p = self.read_floats()
