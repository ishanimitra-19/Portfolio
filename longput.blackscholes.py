...
# ----------------------------
# Core computations (LONG put)
# ----------------------------

def long_put_pl_per_share(premium_paid: float, K: float, ST: float) -> float:
    payoff = max(K - ST, 0.0)
    return payoff - premium_paid


def run_once(S, K, sigma, r, t, steps=252, mu_for_path=None, seed=None):
    theo = black_scholes_put(S, K, sigma, r, t)  # theoretical price (cost)
    mu = r if mu_for_path is None else mu_for_path
    path = gbm_path(S, mu, sigma, steps, t, seed=seed)
    ST = float(path[-1])
    pl_per_share = long_put_pl_per_share(theo, K, ST)  # using theoretical cost
...
def plot_single_path(path: np.ndarray, K: float, pl_per_share: float):
    ...
    ST = path[-1]
    plt.figure()
    plt.title("Terminal Value of a Long Put Option")
...
def plot_cumulative_equity(sampled_contract_pls: np.ndarray, max_points: int = 50_000):
    ...
    plt.figure()
    plt.title("Trading the Long Put Over Time")
...
class App(tk.Tk):
    ...
    def on_run_once(self):
        try:
            ...
            self.write(f"Theoretical put price
